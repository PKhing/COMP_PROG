...
img_dir = path.join('Workshop/Workshop10-Pygame2/source/img')
...
def chooseGameMode():  
    while True:
        ...
        text_surface = font.render("Welcome to SpaceShip!",True,white)
        text_rect = text_surface.get_rect()
        text_rect.midtop = (width//2,height//2-100)
        screen.blit(text_surface,text_rect)

        font = pg.font.Font(font_name, 30)
        text_surface = font.render("Press [e] for easy mode",True,white)
        text_rect = text_surface.get_rect()
        text_rect.midtop = (width//2,height//2-30)
        screen.blit(text_surface,text_rect)

        text_surface = font.render("Press [h] for hard mode",True,white)
        text_rect = text_surface.get_rect()
...
while True:
    ...
    hits = pg.sprite.groupcollide(meteors_small,bullets,True,True)
    for i in hits:
        newmeteor()
        score += 1
    ...
    if hits:
        new_game = True
    hits = pg.sprite.spritecollide(myShip,meteors_small,True)
    if hits:
        new_game = True
